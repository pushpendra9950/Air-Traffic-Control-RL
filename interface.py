...
#this is compute the distance plane should be re routed to when we have action  
REROUTE_DISTANCE = 50

#this is compute the distance awayy from the airport where plane can not reroute 
...
class Main:
    ...
    def __init__(self, qTableFile=None, alpha = 0.5, lamda = 0.9, explore = 0.1 ):
        ...
        # we Initialize a default Sarsa object or with a pre-initialized Q-table
        ...
        #  the Key is the plane ID and the value is the tuple (state, action)
        self.planeHistory = {}
    ...
    def run(self):
        ...
        while (exit == 0):
            if (state == STATE_MENU):
                ...
                # this is  Save the Q table every 25 episodes to save progress
                ...
                # this is  update the explore probability every 10 episodes
    ...
    def trainSarsa(self, aircraft, collidingAircraft, rewards):
        '''
            ...
            Paramaters:
                aircraft                dict,  this is storing  the aircraft object indexed by the plane ids
                collidingAircraft       list, the tuples that contain two plane ids of planes that are close to colliding
                rewards                 dict, this is  stores the rewards that each plane should get indexed by their ids
    ...
    def getState(self, plane1, plane2):
        '''
             this is calculates the state for plane1, given that plane2, is within the
            potential collision radius of plane1.

            Params:
                plane1                          Aircraft object; The plane of interest
