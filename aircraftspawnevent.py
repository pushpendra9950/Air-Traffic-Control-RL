...
class AircraftSpawnEvent:
    ...
    @staticmethod  # this function split the 4 edges L,R,T,R
    def __generateRandomSpawnPoint(screen_w, screen_h):
        side = random.randint(1, 4)                                 
        previous = 7                    
        if side == 1 and side != previous:
            loc = (random.randint(0, screen_w), 0)          # top region
        elif side == 2 and side != previous:
            loc = (screen_w, random.randint(0, screen_h))   # right region
        elif side == 3 and side != previous:
            loc = (random.randint(0, screen_w), screen_h)   #  bottom region
        elif side == 4 and side != previous:
            loc = (0, random.randint(0, screen_h))          #  left region
