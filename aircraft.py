...
class Aircraft:
    ...
    def getLocation(self):      #Return current location
        return np.array(self.location)

    def getDistanceToGo(self):
        return np.linalg.norm(self.destination.getLocation() - self.getLocation())      #Return the distance to the destination

    def getHeading(self):
        ret = 0
        if self.heading < 0:
            ret = 360 + self.heading      #Return current heading
        else:
            ret = self.heading
        return ret
    ...
    def setSpeed(self, newspeed):               # now we will Set speed in pixels per frame
        self.speed = newspeed
##   now we will Set whether I am the selected aircraft or not
    ...
    def draw(self, surface):
        ...
        if(True == True):
            point_list = []                              # we Draw  the lines and waypoints if selected
	...
	# this function will be the Calculate heading based on current position and waypoint
	...
	# this function will be Calculate new location based on current location, heading and speed
	...
	# this function Check whether I have reached the given waypoint
